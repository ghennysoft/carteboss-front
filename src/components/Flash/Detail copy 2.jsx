...
const Detail = () => {
    ...
    const generateVCard = async () => {
        ...
        try {
            ...
            if (item.profession) {
                myVCard.addJobtitle(item.profession);
            }
            if (item.company) {
            ...
            if (item.bio) {
                myVCard.addNote(item.bio);
            }

            // 6. Ajout des coordonnées
            if (item.email) {
                myVCard.addEmail(item.email);
            }
            if (item.phoneNumber) {
                const cleanPhone = item.phoneNumber.replace(/[^\d+]/g, '');
                myVCard.addPhoneNumber(cleanPhone, 'CELL');
            }
            if (item.address) {
                myVCard.addAddress(null, null, item.address, null, null, null, null);
            }

            // 7. Ajout des URLs
            if (item.linkedin?.url) {
                myVCard.addUrl(item.linkedin.url, 'LinkedIn');
            }
            if (item.website?.url) {
                myVCard.addUrl(item.website.url, 'Website');
            }
            if (item.facebook?.url) {
                myVCard.addUrl(item.facebook.url, 'Facebook');
            }
            if (item.instagram?.url) {
                myVCard.addUrl(item.instagram.url, 'Instagram');
            }
            ...
            //             myVCard.addPhoto(resizedBase64, 'JPEG');
